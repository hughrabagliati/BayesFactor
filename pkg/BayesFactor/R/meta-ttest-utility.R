...
meta.ttest.tstat <- function(t,r=NULL,n1,n2=NULL,nullInterval=NULL,rscale, complement = FALSE)
{
  ...
  if(is.null(nullInterval)){
    return(meta.t.bf(t,r,n,nu,rscale=rscale))
  }else{
    return(meta.t.bf(t,r,n,nu,interval=nullInterval,rscale=rscale, complement = complement))
  }
}


meta.t.bf <- function(t,r,N,df,interval=NULL,rscale, complement = FALSE){
  ...
  if(is.null(interval)){
    return(meta.bf.interval(-Inf,Inf,t,r,N,df,rscale))
  }
  ...
  if(any(is.infinite(interval))){
    if(!complement){
      bf = meta.bf.interval(interval[1],interval[2],t,r,N,df,rscale)
    }else{
      if( ( interval[1]==-Inf ) ){
        bf.compl = meta.bf.interval(interval[2],Inf,t,r,N,df,rscale)
      }else{
        bf.compl = meta.bf.interval(-Inf,interval[1],t,r,N,df,rscale)
      }
    }
  }else{
    ...
    bf1 = meta.bf.interval(-Inf,interval[1],t,r,N,df,rscale)
    bf = meta.bf.interval(interval[1],interval[2],t,r,N,df,rscale)
    bf3 = meta.bf.interval(interval[2],Inf,t,r,N,df,rscale)
...
meta.bf.interval <- function(lower,upper,t,r,N,df,rscale){
